...
udp_default_portlist="53,67,68,69,123,135,137-139,161,500,514,520,631,1434"
...
then
    ...
    then
        ...
        for i in $(seq 0 $end_range); do
            ...
            udp_portlist+="${udp_top_100_portlist[$i]}"
...
if [ "$udp_scan" = true ]; then
    udp_option="--udp-ports $udp_portlist"
...
masscan_cmd="masscan $target_specification --ping $router_option -p$tcp_portlist $udp_option --rate $rate -oL $outputbase.masscan"
